...
Several sibling implementations for specialized use-cases are also available:

* `PooledStringWeaver`: Implements essentially exactly the same API as `StringWeaver`, except sourcing its backing buffers from `System.Buffers.ArrayPool<char>.Shared` to minimize allocations. Consider using this if you frequently create short-lived `StringWeaver`s with medium capacities.
